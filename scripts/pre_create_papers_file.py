...
def process_authors_to_string_with_highlighting(
    authors: str, highlight_name: str = "csefalvay"
) -> str:
    authors = authors.split(" and ")
    authors = [author.split(", ") for author in authors]
    authors = [f"{author[0]} {author[1][0]}" for author in authors]
    authors = [
        f"**{author}**" if highlight_name.lower() in author.lower() else author
...
def write_into_file(
    entries_by_year: dict, destination_path: str, destination_div: str
) -> None:
    ...
    md = (
        md[:start]
        + f"<!-- {destination_div} -->\n\n"
        + generate_list_by_year(entries_by_year)
        + md[end:]
    )
