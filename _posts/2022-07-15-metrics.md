...
Under the ROC framework, a perfect model will be represented by the point $(0, 1)$, however, trade-off exists as decreasing the threshold $t$ will improve the TPR at the expences of the FPR.
...
While ROC-AUC is considered to be robusts to imbalance datasets w.r.t. other metrics such as the accuracy.
It is also known to be "over-optimistic" as scoring model performances when the datasets are higly skwed.
...
As shown in Fig. 2a , let assume that the negative examples overlap with the positive onces according to an uniform distribution proportional to the number of positive example.
...
Thus, this example demonstrate how the ROC-AUC is fooled by a high skwe on the label distribution.
