...
Specifically, PR-curves ignores the negative class and only focus on the positive class by replacing the False Positive Rate with the **Precision**.
Precision and Recall are both related only to the positive class; thus PR-curve is inhenertly robusts to impallance dataset.

fig

properties
- no universal baseline
- no meaning in the PR-AUC expect the geometric one. It is not directly related to the F1-score.
- pareto frontier is not convex, thus difficult to 
