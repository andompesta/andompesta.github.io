...
- $x_k$ represents the independent variables, also known as predictors or features.
- $w_k$ are the model parameters establishing a linear relationship between the independent variables and the target. These coefficients indicate the expected change in the dependent variable for a one-unit change in the respective independent variable, with all other variables held constant.
- $\epsilon$ is the error term, which account for the viariability in $y$ that can not be explained by linear model.

If we assume that the only source of error in our model would the difference between the predicted outcome ($\hat{y}$) and the actual value ($y$) of the dependent variable; then learning process would adjust the parameters $w_i$ to minimise the error terms:
...
This sort of model formulation is usually known as the *least-square model* as [@eq-least-square] is called least-square method and [@eq-sum-of-squared-residuals] is the sum-of-sqaured-residual loss.
...
While it is possible to use stochastic gradient descent, a more compact solution to [@eq-sum-of-squared-residuals] can be obtained by linear algebra.
Linear algebra allows for a general formulation that generalize well w.r.t. the number of indipendent variable.
Thus, the same solution can be applied to the bivariante as well as the multivariate case.
Assuming a dataset with composed of $N$ elements, we can rewrite the loss function as:
...
Finally, it is worth mentioning that this solution is known with the term ordinarly least square and can be applied to any model that have a linear formulation w.r.t. the model's parameters (however, we can have any polinomial for the independent variables).
