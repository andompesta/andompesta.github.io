...
As shown in [@fig-least-square], solving [@eq-sum-of-squared-residuals] will give us a model that minimises the distnace between the fitted line and the data points.
...
It is then possible to compute the optimal paramters as:
$$
\begin{align}
...
- $\rmX^T \rmX$ is an $n \times n$ matrix; thus could be invertible;
- $\rmX^T\rvy^T$ is an $n \times 1$ column vector;
- $\rvw^T$ is the $n \times 1$ column vector of unknown parameters.
