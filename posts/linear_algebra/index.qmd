...
1. **transpose**: given a matrix $\rmA \in \real^{m \times n}$, its transpose $\rmA^T$ is obtained ''flipping'' the rows and colums
...
2. **matrix multiplication**: while the summation of 2 matrixes is done element-wise. Matrix multiplication is done row-by-colum and requires matrixes of specific sizes. Given $\rmA \in \real^{m \times n}$ and $\rmB \in \real^{n \times p}$ it is possible to define $\rmC = \rmA \rmB \in \real^{m \times p}$ s.t. $c_{i,j} = \sum_{k=1}^{n} a_{ik} b_{kj}$. In other words, $\rmC$ is a linear combination of the row of $\rmA$ and the colum of $\rmB$.
...
For example, the vectors $\mathbf{i} = \left[
\begin{array}{c}
  1\\
  0
\end{array}
...
\begin{array}{c}
  ...
  1
\end{array}
\right]$ are the most common base vectors for the vectors space $\real^2$. Thus, it is possible to represent a vector $\mathbf{x} = \left[\begin{array}{c}
  3\\
  2
...
![](img/linear_algebra/basic_vectors.png){#fig-base-vector}
...
However, $\mathbf{i}$ and $\mathbf{j}$ are not the only possible basis vectors of $\real^2$.
...
:::{#fig-transformations}
![](img/linear_algebra/transformations.png)
...
Thanks to their properties, it is possible to linearly transform any vector by means to its basis. In other words, given a vector $\rvx = \left[\begin{array}{c} -1\\ 2 \end{array}\right]$ w.r.t. $\rvi$ and $\rvj$ and any lineart transformation $L$.
It is possible to represent $L(\rvx)$ as a function of $L(\rvi)$ and $L(\rvj)$ (formally $L(\mathbf{x}) = -1 L(\mathbf{i}) + 2 L(\mathbf{j})$).
...
Visualization of the linear transformation appled to vector $\rvx$.
:::
...
# Determinant
As linear transformations alter the original vector space, it is important to evaluate by how much the original space is expanded or contracted by a given linear transformation $L$. As shown in [@fig-determinant], the **determinant** define how much the original unit surface is changed by $L$.
...
:::{#fig-determinant}
![](img/linear_algebra/determinant.png){#fig-determinant}

Visualization of the determinant for a initial vector space defined by $\mathbf{i}$ and $\mathbf{j}$ and the vector space obtained after applying the transformation $L$.
:::
...
However, as shown if [@fig-base-change], we can also represent $\rvx$ in terms of different basis vectors $\rvu = \left[\begin{array}{c} 2\\ 1\end{array}\right]$ and $\rvv = \left[\begin{array}{c} -1\\ 1\end{array}\right]$. That is, $\rvx$ can be represented as the linear combination of $\rvu$ and $\rvj$:
...
:::{#fig-base-change}
![](img/linear_algebra/base_change.png)
Visualization of the same vector $\rvx$ represented according to two different basis vectors.
:::
...
:::{#fig-eigen-values}
![](img/linear_algebra/eigen_values.png)

Visualization of one of the eigenvalue of $L$. Note that, $\mathbf{e}_1$ remain on its own span, while a random vector $\mathbf{v}$ is moved away from its original span.
:::


Base on the intuition shown in [@fig-eigen-values] and on the "move from the span" consepts, we can formally define the eigenvalues of a squared matrix $\rmA \in \real^{n \times n}$ as the non-zero vector $\mathbf{e}_i$:
...
Given a squared matrix $\rmA \in \real^{n \times n}$, it is possible to rewrite [@eq-eigenvectors] in matrix form as:

$$
\begin{equation}
\rmA \rmU = \rmU \mathbf{\Lambda}.
\end{equation}
$$ {#eq-eigenvectors-matrix}

Moreover, according to [@eq-cob], using the eigenvectors of $\rmA$ as new basis of $\rmA$ will generate a diagonal matrix of eigenvalues:
...
\rmU \in \real^{n \times n} = \left[\begin{array}{ccc}
  ...
  | & | & | \\
\end{array}\right]$$
is the matrix formed by the eigenvectors of $\rmA$ and 
...
$$\mathbf{\Lambda} \in \real^{n \times n} = \left[\begin{array}{ccc}
  \lambda_1 &  &  \\
   ...
   & & \lambda_n \\
\end{array}\right]$$
is the diagonal matrix formed by the eigenvalues assogiated to the eigenvectors of $\rmA$.
...
which is equivalent to the eigenvector equation [@eq-eigenvectors-matrix] $\rmH \rvx = \lambda \rvx$.
...
where $\rmU \in \real^{m \times m}$ is composed by orthonormal columns ($\rmU^T \rmU = \rmI$), $\rmV \in \real^{n \times n}$ is compesed by orthonormals rows and columns ($\rmV^T\rmV = \rmV \rmV^T = \rmI$), and $\rmS \in \real^{m \times n}$ is a diagonal matrix containing the **singular values** $\sigma_i \geq 0$.
