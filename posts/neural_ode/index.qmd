...
As $f$ defines the evolution of a complex system over every infinitesimal interval of time $\Delta t$, we can define the evolution of such system over time as:
...
Given an inital state $x(t_0)$ and summing up all the infinitesimal time interval from $t_0$ to $T$ it is possible to obtain the final state $x(T)$ as:
$$
\begin{align}
...
An ODE is generally defined in a functional form as in [@eq-ode].
Solving such type of equations involve to computetion of the antiderivative of $f$, in other words the integral of $f$:
...
As the integral of any function involve the introduction of an arbitrary constant $c$, the solution of [@eq-ode-solution] is, in most cases, not unique.
In other words, an ordinary differential equation specifies a rule for the derivative of an unknown function, but it does not specify the value of the function itself. 
To uniquely determine the solution, there is the need to specify the *initial condition* $x(t_0)$ from which we start to evolve the system.
Note that, given the inital state $x(t_0)$ and a set of points in time $\{ t_0, ..., T\}$, the objective is to obtain the state solution $x(t_0:T) \equiv \{ x(t_0), ..., x(T)\}$.
