...
As $f$ defines the evolution of a complex system over small interval of time $\Delta t$, we can describe the evolution of the system over descreate time steps as:
...
ODEs are generally expressed as in [@eq-ode], where only the derivate of a function is specified.
Solving such equations involves computing the antiderivative of $f$, in other words, the integral of $f$:
...
Since the integral of any function introduces an arbitrary constant $c$, the solution of [@eq-ode-solution] is, in most cases, not unique.
...
Most commonly, given the inital state $x(t_0)$ and a set of points in time $\{ t_0, ..., T\}$, the objective of solving an ODE is to obtain the solution states $x(t_0:T) \equiv \{ x(t_0), ..., x(T)\}$ that evolves according to [@eq-ode].

### Solving a First-Order Linear Ordinary Differencial Equation

When an ODE is defined by a firs-order polynomial, is called First-Order Linear ODE.
These type of ODEs are used to express exponential changes in the dependent variable and are formulated as:

$$
\begin{align}
\frac{\partial x(t)}{\partial t} = \lambda x(t).
\end{align}
$$

### Numerical Solvers

Unfortunetly, solving integral analytically is possible only for a limited amount of differential functions.
To overcome the complexity of simulating the evolution of complex system over time, numerical approaches are used in practice to approximate the solution up to a tolerable error.
One of the simples solvers is the **Euler solver** which approximates the solution of an ODE by iteratively adding the product of the step size $h$ (representing a small change in time) and the derivative to the current estimate of the solution:

$$
x(t+1) = x(t) + h \cdot f(x(t), t).
$$


<!-- introduce VDP example  -->


For example, conside the dynamics of a VDP oscillator [@van1960theory]: a non-conservative oscillating system with non-linear damping defined by a parametric 2D time-invariant order differential equation:
...
x_2(t) \\
u\big(1-x_2(t)^2\big) x_2(t) - x_1
\end{array} \right]
$$ {#eq-vdp-ode}

where $x_1(t)$ and $x_2(t)$ are the two dimention of the column vector $\mathbf{x} \in \mathbb{R}^2$. Solving [@eq-vdp-ode] analytically is not as symple as in the previous example, yet Cagatay Yildiz [@probabilistic_ai_school_neural_ode] [@cagatayyildizNeuralOrdinary] inllustrates how the Euler solver could easy solve such type of systems. Here I will follow his code and visualizations.
...
Neural ODEs [@DBLP:journals/corr/abs-1806-07366] are a family of ODEs for which the vector-field $f(x_t, t)$ is defined by a neural network. As such, $f(x_t, t)$ is both differentiable and learnable. Thus, given a set of observation $y_{0:N}$ from a unknwon dynamical system, we can used it to learn a model of the evolution of the system's dynamics.
